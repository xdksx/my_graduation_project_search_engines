...
//对数据库的操作
     DB *dbp;
       ...
       ret=dbp->open(dbp,NULL,"../../search_server_v05_arrange/index_builded.db",NULL,DB_BTREE,flags,0);
       if(ret!=0)
	...
	//   cout<<"------提取网页的结果----"<<endl;
	  ...
	  //  cout<<"--------------得到的建立在内存中的索引------------"<<endl;
       ...
       post_index.clear(); 
       cout<<"-------------get------------"<<endl;
       build_post_index_m(d,length,pageid_i);
    //   print_post_index();  
     ...
     //   cout<<(*it).first<<"word_count"<<word_count<<endl;
		...
		//从数据库中取得该词元记录
		 ...
		 //  buffer_get=new char[1000000000];
		   buffer_get=(char *)malloc(5000000);//这里不知道分配多少空间,唉,这里还是设置成一个宏吧
		//   char  *fr=buffer_get;
		   int buffer_size=5000000;
		   memset(buffer_get,0,5000000);
	    	int word_size=(*it).first.size();//取得词元的大小(作为key的大小)
	    	const int words_length=word_size+1;
	    	char wordss[words_length];
	    	wordss[word_size]='\0';
	   // 	char *words=NULL;
		 //   words=(char *)malloc(word_size+100);//key的填充要用char *//注意这里的+100调试了一两个小时错误的原因,
		...
		//  cout<<"notfound，so puting new record......"<<endl;
		   pagecount_exist_map[(*it).first]=1;
		     int all;
